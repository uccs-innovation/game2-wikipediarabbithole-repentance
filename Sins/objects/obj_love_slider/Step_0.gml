...
if (mouse_check_button_pressed(mb_left) and (obj_gameManager.fight_winner == 0 or obj_gameManager.fight_winner == -1)) {	
	var button_x = x+sprite_width*obj_gameManager.love_value;
	var button_y = y;
...
if selected {
	obj_gameManager.love_value = clamp((mouse_x-x)/sprite_width, 0, max_value);
	event_user(0);
}